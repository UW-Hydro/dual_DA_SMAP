...
**********************************************************************/
  ...
  if ( argc < 9 || argc > 10 ) {
    ...
    fprintf(stderr,"\n\t<num start | start file> number of random simulation parameter sets\n\t\twith which to start the test population or the name of restart\n\t\tfile.  If the restart filename is followed by a `+`, the model \n\t\twill be reapplied using the restart parameters (default uses \n\t\trestart statistics and does not rerun the model).\n");
  ...
  if ( argv[1][0] >= 48 && argv[1][0] <= 57 ) {
    ...
    /* restart selected */
    if ( ( pos = index( argv[1], '+' ) ) != NULL ) {
      RESTART_FRESH = TRUE;
      *pos = '\0';
    }
    else RESTART_FRESH = FALSE;
    RESTART = TRUE;
  ...
  if ( RESTART ) restart_optimization(restart_file, fopti, set, param_lim,
				      ...
				      &prob, storefiles, RESTART_FRESH, 
				      runstr, labelstr, storedir);
...
void restart_optimization(char        *filename,
  ...
  for ( setcnt = 0; setcnt < N_SET; setcnt++ ) {
    ...
    for ( test = 0; test < N_TEST_FUNCS; test++ ) {
      ...
      if ( !RESTART_FRESH )
	fprintf(fopti, "\t%f", set[setcnt].f[test]);
    }
    if (storefiles) {
      fscanf(f, "%i %i", &set[setcnt].rank, &set[setcnt].soln_num);
      //set[setcnt].soln_num *= -1;
      if ( set[setcnt].soln_num > maxnum ) maxnum = set[setcnt].soln_num;
      if ( !RESTART_FRESH )
	fprintf(fopti, "\t)\t%i\t%i\n", set[setcnt].rank, set[setcnt].soln_num);
    }
    else {
      fscanf(f, "%i", &set[setcnt].rank);
      if ( !RESTART_FRESH )
	fprintf(fopti, "\t)\t%i\n", set[setcnt].rank );
    }
    if ( RESTART_FRESH ) {
      // Rerun the model for the given restart parameter sets
      set[setcnt].y = solve_model(set[setcnt].p, set[setcnt].f, runstr, 
				  param_lim, labelstr, fopti, storedir, 
				  storefiles);
      set[setcnt].soln_num = SOLVE_CNT;
  ...
  if ( ! RESTART_FRESH )
    if ( maxnum > 0 ) SOLVE_CNT = maxnum+1;
