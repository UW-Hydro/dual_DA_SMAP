...
**********************************************************************/
  ...
  /* --- process arguments --- */
  if ( argc != 10 ) usage(); /* AWW-2014 -- upped by 1  */
  ...
  if ( RESTART )
    ...
    random_start_optimization(ran2seed);
...
/* Function returns maximum rank value for a set of runs  */
int rank(ITEM *set, int size) {
  ...
  /* BUG #06 -- this has a corner case where it doesn't assign the current rank to anything,
  ...
  do {
	...
	//        printf("set rank matches loop rank %d\n",rank);
	    ...
	    //            printf("run %d dominated by run %d, deranking it\n",test,i);
          ...
          }  // AWW added brackets to make this easier to track
	}
      }
    }
