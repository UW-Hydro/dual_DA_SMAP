...
''' This script run VIC with "truth" SWE and "openloop" lower layer sm states,
    and original forcing.

    Usage:
        $ python test.truth_swe.orig_P.py <gen_synth_config_file> <mpi_proc>
'''
...
# ============================================================ #
# Run VIC with "truth" SWE states and original forcings
# ============================================================ #
# --- Generate state files with "truth" SWE and openloop other states --- #
...
for t, time in enumerate(meas_times):
    ...
    ds_state_openloop = xr.open_dataset(os.path.join(
        ...
        'state.openloop.{}_{:05d}.nc'.format(
                time.strftime('%Y%m%d'),
                time.hour*3600+time.second)))
    # Replace swe with truth
    ds_state_openloop['STATE_SNOW_WATER_EQUIVALENT'][:] = \
        ds_state_truth['STATE_SNOW_WATER_EQUIVALENT'][:]
...
print('Run VIC with "truth" SWE states and orig. forcing...')
