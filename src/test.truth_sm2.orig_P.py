...
''' This script run VIC with "truth" sm2 and "openloop" lower layer sm states,
    and original forcing.

    Usage:
        $ python test.truth_sm2.orig_P.py <gen_synth_config_file> <mpi_proc>
'''
...
basedir = setup_output_dirs(
    ...
    mkdirs=['test.truth_sm2_orig_forcing'])['test.truth_sm2_orig_forcing']
...
# ============================================================ #
# Run VIC with "truth" sm2 states and original forcings
# ============================================================ #
# --- Generate state files with "truth" sm2 and openloop other states --- #
...
for t, time in enumerate(meas_times):
    ...
    ds_state_openloop = xr.open_dataset(os.path.join(
        ...
        'state.openloop.{}_{:05d}.nc'.format(
                time.strftime('%Y%m%d'),
                time.hour*3600+time.second)))
    # Replace sm2 with truth
    ds_state_openloop['STATE_SOIL_MOISTURE'].loc[:, :, 1, :, :] = \
        ds_state_truth['STATE_SOIL_MOISTURE'].loc[:, :, 1, :, :]
