...
# --- Calculate ensemble mean bias wrt. openloop --- #
da_bias = (da_EnKF_mean - da_openloop).mean(dim='time')  # [lat, lon], unit: mm
# Option 1: calculate mean bias first, then normalize by openloop mean
da_bias_relative1 = da_bias / da_openloop.mean(dim='time')  # [lat, lon], unit: -
# Option 2: calculate relative bias at each time step, then take temporal mean
da_bias_relative2 = ((da_EnKF_mean - da_openloop) / da_openloop).mean(dim='time')  # [lat, lon], unit: -

# --- Plot maps --- #
# Absolute bias
fig = plt.figure(figsize=(14, 7))
cs = da_bias.plot(add_colorbar=False, cmap='bwr_r', vmin=-0.03, vmax=0.03)
cbar = plt.colorbar(cs, extend='both').set_label('Bias (mm/mm)', fontsize=20)
plt.title('sm1, absolute bias of ensemble mean w/o update wrt. openloop', fontsize=20)
fig.savefig(os.path.join(output_dir, 'bias.no_update.sm1.png'), format='png')

# Relative bias option 1
fig = plt.figure(figsize=(14, 7))
cs = (da_bias_relative1 * 100).plot(add_colorbar=False, cmap='bwr_r',
      vmin=-15, vmax=15)
cbar = plt.colorbar(cs, extend='both').set_label('Relative bias (%)', fontsize=20)
plt.title('sm1, relative bias of ensemble mean w/o update wrt. openloop \n' \
          '(normalized by openloop mean)', fontsize=20)
fig.savefig(os.path.join(output_dir, 'bias_relative1.no_update.sm1.png'), format='png')

# Relative bias option 2
fig = plt.figure(figsize=(14, 7))
cs = (da_bias_relative2 * 100).plot(add_colorbar=False, cmap='bwr_r',
      vmin=-15, vmax=15)
cbar = plt.colorbar(cs, extend='both').set_label('Relative bias (%)', fontsize=20)
plt.title('sm1, relative bias of ensemble mean w/o update wrt. openloop \n' \
          '(normalized at each timestep by openloop)', fontsize=20)
fig.savefig(os.path.join(output_dir, 'bias_relative2.no_update.sm1.png'), format='png')
...
# Absolute bias
fig = plt.figure(figsize=(14, 7))
cs = da_bias.plot(add_colorbar=False, cmap='bwr_r',
                  vmin=-0.02, vmax=0.02)
cbar = plt.colorbar(cs, extend='both').set_label('Bias (mm/mm)', fontsize=20)
plt.title('sm2, absolute bias of ensemble mean w/o update wrt. openloop', fontsize=20)
fig.savefig(os.path.join(output_dir, 'bias.no_update.sm2.png'), format='png')
...
cs = (da_bias_relative1 * 100).plot(add_colorbar=False, cmap='bwr_r',
                                    vmin=-8, vmax=8)
cbar = plt.colorbar(cs, extend='both').set_label('Relative bias (%)', fontsize=20)
plt.title('sm2, relative bias of ensemble mean w/o update wrt. openloop \n' \
          '(normalized by openloop mean)', fontsize=20)
fig.savefig(os.path.join(output_dir, 'bias_relative1.no_update.sm2.png'), format='png')
...
plt.title('sm2, relative bias of ensemble mean w/o update wrt. openloop \n' \
          '(normalized at each timestep by openloop)', fontsize=20)
fig.savefig(os.path.join(output_dir, 'bias_relative2.no_update.sm2.png'), format='png')
...
cbar = plt.colorbar(cs, extend='both').set_label('Bias (mm/mm)', fontsize=20)
plt.title('sm3, absolute bias of ensemble mean w/o update wrt. openloop', fontsize=20)
fig.savefig(os.path.join(output_dir, 'bias.no_update.sm3.png'), format='png')
...
cs = (da_bias_relative1 * 100).plot(add_colorbar=False, cmap='bwr_r',
                                    vmin=-50, vmax=50)
cbar = plt.colorbar(cs, extend='both').set_label('Relative bias (%)', fontsize=20)
plt.title('sm3, relative bias of ensemble mean w/o update wrt. openloop \n' \
          '(normalized by openloop mean)', fontsize=20)
fig.savefig(os.path.join(output_dir, 'bias_relative1.no_update.sm3.png'), format='png')
...
plt.title('sm3, relative bias of ensemble mean w/o update wrt. openloop \n' \
          '(normalized at each timestep by openloop)', fontsize=20)
fig.savefig(os.path.join(output_dir, 'bias_relative2.no_update.sm3.png'), format='png')
