...
# Simulated flow forced by true prec forcing
flow_true_force_nc = cfg['ROUTE']['flow_true_force_nc']
...
for site in dict_sites.keys():
    ...
    ts_true_force = df_true_force_daily.loc[:, site]
    ts_usgs = ts_usgs[ts_openloop.index].dropna()
    ts_openloop = ts_openloop[ts_usgs.index].dropna()
    ts_true_force = ts_true_force[ts_usgs.index].dropna()
...
#        site=site, time=ts_usgs.index).transpose('time', 'N').values
    ...
    rmse_true_force = rmse(ts_true_force, ts_usgs)
    per_RMSE = (1 - rmse_ensMean / rmse_openloop) * 100
    per_RMSE_true_force = (1 - rmse_true_force / rmse_openloop) * 100
    # PER(logRMSE)
...
#    psr = (1 - crps_ens / crps_zero_update) * 100
    ...
    kge_true_force = kge(ts_true_force, ts_usgs)
    kge_improv = kge_ensMean - kge_openloop
    kge_improv_true_force = kge_true_force - kge_openloop
    ...
    for i in range(N):
        ...
        ts.plot(color='blue', alpha=0.2, label='_nolegend_')
    (ts_ensMean/1000).plot(color='blue', linewidth=2, label="Ensemble")
    # USGS
    (ts_usgs / 1000).plot(color='black', label="USGS")
    # Openloop
    (ts_openloop / 1000).plot(
        color='magenta', style='-', label="Openloop")
    # True force
    (ts_true_force / 1000).plot(
        color='green', style='-', label="Forced by true precip.")
...
#             verticalalignment='top', transform=ax.transAxes, fontsize=16)
    ...
    plt.text(0.7, 0.6,
             "Forced by true prec.: \n" + \
             "all-time PER(RMSE) = {:.1f}%\n".format(per_RMSE_true_force) + \
             "all-time PER(logRMSE) = {:.1f}%\n".format(per_logRMSE_true_force) + \
             "all-time KGE_improve = {:.2f}\n".format(kge_improv_true_force),
             horizontalalignment='left',
             verticalalignment='top', transform=ax.transAxes, fontsize=16)
    # Make plot better
